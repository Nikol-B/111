...
import { sendFormData, closeErrorModal } from './modules/ajax';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  closeErrorModal(); // Закрытие модального окна
  ...
  if (phoneInput) {
    ...
    phoneMask.mask(phoneInput);
     ...
     // Проверка количества цифр в номере после ввода
     phoneInput.addEventListener('input', () => {
      const value = phoneInput.value.replace(/\D/g, ''); // Удаляем все нецифровые символы
      if (value.length !== 12) {
        phoneInput.setCustomValidity('Требуется ввести 12 цифр');
      } else {
